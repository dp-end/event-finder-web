  ...
  Future<void> _girisYap() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final token = ApiClient.extractToken(userData);
        if (token == null) {
          throw Exception('Giris basarili ancak oturum anahtari alinamadi.');
        }
        CampusHubApp.userNotifier.value = userData;
        // JWT token'i kaydet
        CampusHubApp.tokenNotifier.value = token;
        // Kullanıcı tipini backend'den al (roles listesine göre otomatik belirlendi)
        final userType = ApiClient.userTypeFrom(userData, fallback: _seciliTip);
        ...
        final mesaj = ApiClient.errorMessageFromResponse(response, 'E-posta veya sifre hatali.');
        if (mounted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(e.toString().contains('SocketException') ? 'Sunucuya baglanilamadi. Backend acik mi?' : e.toString())),
        );
      }
    } finally {
