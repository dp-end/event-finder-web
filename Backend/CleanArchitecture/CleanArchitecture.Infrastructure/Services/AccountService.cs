    ...
    public class AccountService : IAccountService
    {
        ...
        public async Task<AuthenticationResponse> AuthenticateAsync(AuthenticationRequest request, string ipAddress)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.UserType)
                && request.UserType.ToLower() != response.UserType)
            {
                throw new ApiException(isClub
                    ? "Bu hesap bir kulup hesabidir. Lutfen Kulup Girisi secin."
                    : "Bu hesap bir ogrenci hesabidir. Lutfen Ogrenci Girisi secin.");
            }
            if (isClub)
            {
                var club = _context.Clubs.FirstOrDefault(c => c.Id == user.Id || c.AdminUserId == user.Id);
                response.ClubId = club?.Id;
            }
